...
# Match measurement values from power meter with experiment suite IDs for HPL 

# number of experiment suites per run: 3
...
## experiment suite ids: 344-346 (key id in table ExperimentSuite)
...
# append number of active rpis to every line in file and write to result file results/energymatch`date +%y%m%d`.txt
cat out.txt | while read line
do 
	echo "$line $active_rpis" >> results/energymatch`date +%y%m%d`.txt  
done  

# same procedure for other split files 
for i in out{1..3}.txt
do 
	# get content of last line (number of active RPis)
