...
half=$((linecount / 2))
echo "Half of linecount: $half"
# split -l $half ../sources/hpl-2.1_db`date +%y%m%d`.txt
split -l 3 ../sources/hpl-2.1_db`date +%y%m%d`.txt
...
# split tmp file 1 into 3 subfiles
split -l 1 ../sources/tmp1.txt plot1

# split tmp file 2 into 3 subfiles
split -l 1 ../sources/tmp2.txt plot2

# insert number of active RPis as last column in tmp split files
# write to plot files 

sed -e 's/$/ 16/' plot1aa > ../sources/hplplot1.txt
sed -e 's/$/ 8/' plot1ab >> ../sources/hplplot1.txt
sed -e 's/$/ 4/' plot1ac >> ../sources/hplplot1.txt
...
sed -e 's/$/ 8/' plot2ab >> ../sources/hplplot2.txt
sed -e 's/$/ 4/' plot2ac >> ../sources/hplplot2.txt
