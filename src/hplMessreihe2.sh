...
# Run HPL on 16 RPi-Nodes, 17 RPi-Nodes powered
...
# create output file                                                                 
touch results/hpl-2.1_shutdown`date +%y%m%d`.txt

# shutdown inactive RPis                                                           
# for host in pi16 pi14 
# do
...
# log number of active RPis/powered RPis to results/hpl-2.1_shutdown`date +%y%m%d`.txt
echo "Active RPis: $n/Powered RPis: $m" >> results/hpl-2.1_shutdown`date +%y%m%d`.txt

# log experiment suite id to results/hpl-2.1_shutdown`date +%y%m%d`.txt                                                                                                             
echo "Experiment suite id: $myid" >> results/hpl-2.1_shutdown`date +%y%m%d`.txt

# start benchmark on 16 RPis, log to results/hpl-2.1_shutdown`date +%y%m%d`.txt                        
# mpiexec -n $n -machinefile /srv/libraries/etc/mpich-3.0.4-shared/machinefile_latest -wdir /srv/benchmarks/bin/hpl-2.1/messung3 /srv/benchmarks/bin/hpl-2.1/messung3/xhpl >> results/hpl-2.1_shutdown`date +%y%m%d`.txt
...
# Run HPL on 8 RPi-Nodes, 9 RPi-Nodes powered
...
# shutdown inactive RPis                                                           
# for host in pi05 pi20 pi19 pi18 pi17 pi15 pi13 pi12 pi11
# do
#	ssh $host 'shutdown -hP 0'
# re-attach terminal to read new stream from STDIN
#	term="/dev/$(ps -p$$ -o tty="")"
...
# start benchmark on 8 RPis, log to results/hpl-2.1_shutdown`date +%y%m%d`.txt                        
# mpiexec -n $n -machinefile /srv/libraries/etc/mpich-3.0.4-shared/machinefile_latest -wdir /srv/benchmarks/bin/hpl-2.1/messung2 /srv/benchmarks/bin/hpl-2.1/messung2/xhpl >> results/hpl-2.1_shutdown`date +%y%m%d`.txt
...
# shutdown inactive RPis                                                           
# for host in pi10 pi09 pi08 pi07 
# do
#	ssh $host 'shutdown -hP 0'
# re-attach terminal to read new stream from STDIN
#	term="/dev/$(ps -p$$ -o tty="")"
...
# start benchmark on 4 RPis, log to results/hpl-2.1_shutdown`date +%y%m%d`.txt                        
# mpiexec -n $n -machinefile /srv/libraries/etc/mpich-3.0.4-shared/machinefile_latest -wdir /srv/benchmarks/bin/hpl-2.1/messung1 /srv/benchmarks/bin/hpl-2.1/messung1/xhpl >> results/hpl-2.1_shutdown`date +%y%m%d`.txt
...
# one split file per experiment suite = 6 files                                                                                                      
awk '/Experiment/{n++}{print >"out" n ".txt" }' results/hpl-2.1_db`date +%y%m%d`.txt

for file in out{1..6}.txt
do
