...
% Name, Beschreibung
Die Messung des Stromverbrauchs erfolgt mit dem Strommessger"at Energenie EGM-PWM-LAN (vgl. \url{http://www.elv.de/energenie-egm-pwm-lan-energiekosten-messgeraet-mit-lan-schnittstelle.html}), im Folgenden als \textit{Energenie} bezeichnet. Es wird zwischen Steckdose und den Bramble gesteckt und "uber ein LAN-Kabel mit dem Netzwerk verbunden. 

% Aktivierung
F"ur den vorliegenden Versuchsaufbau war dem Energenie eine statische IP-Adresse zugewiesen und ein DNS-Eintrag erstellt worden. Damit ist es m"oglich, das Ger"at im lokalen Netzwerk zu erreichen und auf die ermittelten Messwerte zuzugreifen. 

% Datenspeicherung 
Dazu bietet das Energenie mehrere M"oglichkeiten, darunter eine Webbrowser-Oberfl"ache und ein Windows-Programm. Damit die Software das Ger"at erkennt, muss es sich im selben Subnetz befinden wie der Rechner, von dem aus zugegriffen wird.  

% Datenzugriff 
F"ur den Versuchsaufbau erwies sich die Webbrowser-Oberfl"ache als weniger geeignet, da sie haupts"achlich Konfigurationsfunktionen bietet. 
