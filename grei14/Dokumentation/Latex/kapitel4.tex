...
Das folgende Kapitel dient der Bewertung und Einordnung der ermittelten Ergebnisse. Im Fokus steht das Skalierungsverhalten des Bramble. Abschlie\ss end werden Grenzen des Versuchsaufbaus diskutiert.   

\section{HPL: Performance}\label{Interpretation-Linpack}
% TODO Ergänzung erwartete Messwerte
Wie in Kapitel \ref{Ergebnisse-HPL} dargestellt, verh"alt sich das Quadrat der Problemgr"o\ss e proportional zur Gr"o\ss e des Gesamthauptspeichers. Somit ist bei Hinzunahme von Ressourcen in Form von RPi-Knoten ein linearer Anstieg der Performance zu erwarten. Das gilt ebenfalls f"ur die Ausf"uhrungsdauer. Die Diagramme \ref{fig:hpl1} und \ref{fig:hpl2} best"atigen diese Erwartung: Der Anstieg der Performance ist nahezu linear und betr"agt im Mittel 0.031 GFLOPs pro zus"atzlicher Ressource. Die maximale Abweichung hiervon betr"agt 0.008 GFLOPs. Der Anstieg der Ausf"uhrungdauer ist nahezu linear und betr"agt im Mittel 13.28 s. Die maximale Abweichung hiervon betr"agt 8.24 s.

Der Versuchsaufbau und die Funktionsweise des Benchmarks lassen vermuten, dass sich kein signifikanter Unterschied in Performance und Ausf"uhrungsdauer zeigt, wenn nicht an der Programmausf"uhrung beteiligte RPi-Knoten heruntergefahren werden. 

Die Diagramme \ref{fig:hpl5} und \ref{fig:hpl6} best"atigen diese Erwartung: Messreihe 1 und Messreihe 2 werden in den Grafiken nahezu identisch dargestellt. Die Ausf"uhrungsdauer in Messreihe 2 ist geringf"ugig h"oher als in Messreihe. Die maximale Abweichung betr"agt dabei 0.63 s, die mittlere Abweichung 0.41 s. Die Performance ist in Messreihe 2 f"ur $n=4$ und $n=16$ RPi-Knoten geringf"ugig niedriger als in Messreihe 1. Die Abweichung betr"agt dabei 0.001 GFLOPs, die mittlere Abweichung 0.001 GFLOPs (gerundet auf drei Nachkommastellen an Hand der Messgenauigkeit, vgl. Kap. \ref{Ergebnisse-HPL}). 

Es l"asst sich schlussfolgern, dass das Herunterfahren nicht mehr aktiver RPi-Knoten keine signifikanten Auswirkungen auf Performance und Ausf"uhrungsdauer von HPL auf dem Bramble hat. Das Skalierungsverhalten in Messreihe 1 und Messreihe 2 kann damit als gleich bezeichnet werden. 

\section{STREAM: Performance}\label{Interpretation-Stream}

Die Funktionsweise des Benchmarks l"asst erwarten, dass das Hinzunehmen von Ressourcen einen linearen Anstieg der Performance zu Folge hat. McCalpin schreibt hierzu: 
\begin{quote}
\onehalfspacing
...
\end{quote}
F"ur die Ausf"uhrungsdauer auf jeder einzelnen CPU ist ein konstantes Verhalten zu erwarten. Die Diagramme \ref{fig:stream1} und \ref{fig:stream2} best"atigen die Erwartungen f"ur $n\leq 17$ RPi-Knoten: Der Anstieg der Performance bei Hinzunahme von RPi-Knoten ist nahezu linear und betr"agt im Mittel 245.9 MB/s (Copy), 205.1 MB/s (Scale), 275.5 MB/s (Add) bzw. 266.9 MB/s (Triad). Die maximale Abweichung hiervon betr"agt 54.2 MB/s (Copy), 19.7 MB/s (Scale), 25.9 MB/s (Add) bzw. 25.6 MB/s (Triad). Bei der Ausf"uhrungsdauer zeigen sich geringe Abweichungen von maximal 0.001525 s (Copy), 0.002114 s (Scale), 0.002463 s (Add) und 0.003793 s (Triad). 

Das Skalierungsverhalten f"ur $n\leq 17$ RPi-Knoten kann somit als erwartungsgem"a\ss\ bezeichnet werden. 
...
Zwei Ursachen sind denkbar: Systemzeit und Netzwerk. 
